...
L.Control.Ruler = L.Control.extend({
  ...
  _toggleMeasure: function() {
    ...
    if (this._active) {
      ...
      L.DomEvent.on(window.document, "turnOffMapRuler", this._toggleMeasure, this)
    }
    if (!this._active) {
      ...
      L.DomEvent.off(window.document, "turnOffMapRuler", this._toggleMeasure, this)
    }
  },
  _clicked: function(e) {
    // При первом клике отключаем скролл и зумконтрол
    if (!this._clickCount) {
    ...
    // Если точка первая, то добавляем просто маркер без тултипа, иначе маркер с тултипом с расстоянием и линию
    if (!this._clickCount || e.latlng.equals(this._clickedPoints[this._clickedPoints.length - 2])) {
      ...
      let text = `<b>${this._totalLength.toLocaleString("ru-RU", { maximumFractionDigits: this.options.lengthUnit.decimal })}</b>`
      L.circleMarker(this._clickedLatLong, this.options.circleMarker)
          .bindTooltip(text, { permanent: true, direction: "top", className: "leaflet-ruler__result-tooltip" })
  ...
  _moving: function(e) {
    if (this._clickedLatLong) {
      L.DomEvent.off(this._container, "click", this._toggleMeasure, this)
      L.DomEvent.on(this._container, "click", this._closePathAndToggleMeasure, this)
  ...
  _closePath: function(e) {
    ...
    L.DomEvent.off(this._container, "click", this._closePathAndToggleMeasure, this)
    L.DomEvent.on(this._container, "click", this._toggleMeasure, this)
    this._active = false
    this._toggleMeasure()
    if (e) {
